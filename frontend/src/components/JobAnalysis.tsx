...
const JobAnalysis: React.FC = () => {
  ...
  const downloadCoverLetterPDF = async (coverLetterText: string, analysis: AnalysisResult) => {
    try {
      ...
      if (!printWindow) {
        alert('Please allow popups to download the cover letter');
        return;
      }
      ...
      const htmlContent = `
        ...
        <head>
          ...
          <style>
            ...
            @media print {
              body { margin: 0.5in; }
              .footer { position: fixed; bottom: 0.2in; }
            }
          </style>
        </head>
        <body>
          <div class="header">
            <h1>Cover Letter</h1>
            <p>Match Score: ${analysis.match_score}% | Generated: ${new Date().toLocaleDateString()}</p>
          </div>
          <div class="content">${coverLetterText}</div>
          <div class="footer">
            Generated by JobsMatch AI | AI Model: ${analysis.ai_model}
          </div>
        </body>
        </html>
      `;
